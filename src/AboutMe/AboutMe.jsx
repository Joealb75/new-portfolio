      ...
      <div className="w-full max-w-3xl px-4 md:pl-5 pt-4" id="about">
        <div className="flex justify-between items-center w-full">
          ...
          <button
            ...
            className={`relative w-12 h-6 flex items-center bg-gray-400 rounded-full p-1 transition-all ${
              isAboutOpen ? "bg-green-500" : "bg-gray-600"
            }`}
          >
            <div
              className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-all ${
                isAboutOpen ? "translate-x-6" : "translate-x-0"
              }`}
            />
          </button>
        </div>
        ...
        <div
          className={`transition-all duration-500 overflow-hidden ${
            isAboutOpen ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"
          }`}
        >
          <p className="text-xs text-gray-400 pt-1">
            <i className="fas fa-clock mr-2"></i>2 minute read
          </p>
          ...
          <p className="text-gray-400 text-sm">
            ...
            "Hollywood makes coding seems so complicated, is it really that
            ...
            accepted into Nashville Software School intensive 6 month bootcamp
            for full-stack developers with a start date of 02/12/24. For the
            next 6 months, Monday-Friday I spent infront of my computer in zoom
